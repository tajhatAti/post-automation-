...
Features:
- Video/Photo/Doc queue system
- BDT timezone schedule (multiple slots)
- Multi-channel posting
- Custom caption (HTML supported)
- Custom inline buttons (up to 3)
- Force post
- Queue management
- 24/7 on Render.com + UptimeRobot
"""
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# DATABASE (JSON-based, simple & persistent)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
db_lock = threading.Lock()

DEFAULT_DB = {
    "queue": [],           # [{"file_id": "...", "type": "video/photo/doc", "ts": "..."}]
    "schedules": [],       # [{"time": "07:30", "count": 4}]
    "dest_channels": [],   # ["@ch1", "@ch2"]
    "caption": "",         # HTML caption
    "buttons": [],         # [{"text": "...", "url": "..."}]
    "total_posted": 0,
    "last_check_min": -1,
    "state": {}            # admin conversation states
}
...
def build_keyboard(buttons):
    ...
    rows = []
    for b in buttons:
        rows.append([types.InlineKeyboardButton(b["text"], url=b["url"])])
...
def send_dashboard(chat_id):
    db = get_db()
    q_count   = len(db["queue"])
    sch_count = len(db["schedules"])
    ch_count  = len(db["dest_channels"])
    total     = db["total_posted"]
    cap_prev  = (db["caption"] or "<i>(সেট নেই)</i>")[:80]

    per_day = sum(s["count"] for s in db["schedules"])
    days_left = (q_count // per_day) if per_day > 0 else 0
    ...
    bot.send_message(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📦 কিউ: <b>{q_count}</b> টি মিডিয়া\n"
        f"📅 শিডিউল: <b>{sch_count}</b> টি স্লট\n"
        f"📢 চ্যানেল: <b>{ch_count}</b> টি\n"
        f"📬 মোট পোস্ট: <b>{total}</b> টি\n"
        f"⏳ আনুমানিক: <b>{days_left}</b> দিন\n\n"
        f"✍️ ক্যাপশন:\n{cap_prev}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━",
        reply_markup=kb
    )
...
def cmd_help(message):
    bot.send_message(
        ...
        "/del_slot 1 — শিডিউল মুছো (নম্বর দিয়ে)\n"
        "/schedules — শিডিউল লিস্ট\n"
        "/set_caption — ক্যাপশন সেট\n"
        "/add_button — বাটন যোগ\n"
        "/del_button 1 — বাটন মুছো\n"
        "/buttons — বাটন লিস্ট\n"
        "/cancel — যেকোনো state বাতিল\n\n"
        "📎 <b>মিডিয়া যোগ:</b>\n"
        "যেকোনো video/photo/doc সরাসরি পাঠাও — কিউতে যাবে\n\n"
        "━━━━━━━━━━━━━━━━━━━━━"
    )
...
def cmd_status(message):
    ...
    per = sum(s["count"] for s in sch)
    now = bdt_time_str()

    sch_lines = "\n".join([f"  • <code>{s['time']}</code> → {s['count']} টি" for s in sch]) or "  <i>(নেই)</i>"
    ch_lines  = "\n".join([f"  • <code>{c}</code>" for c in chs]) or "  <i>(নেই)</i>"
    ...
    bot.send_message(
        ...
        f"🕒 BDT এখন: <b>{now}</b>\n"
        f"📦 কিউ: <b>{q}</b> টি\n"
        f"📅 প্রতিদিন পোস্ট: <b>{per}</b> টি\n"
        f"⏳ আনুমানিক: <b>{q // per if per else '∞'}</b> দিন\n"
        f"📬 মোট হয়েছে: <b>{db['total_posted']}</b> টি\n\n"
        f"📢 <b>চ্যানেল:</b>\n{ch_lines}\n\n"
        f"🕒 <b>শিডিউল:</b>\n{sch_lines}\n\n"
        f"🔘 <b>বাটন:</b> {len(db['buttons'])} টি\n"
        f"━━━━━━━━━━━━━━━━━━━━━"
    )
...
def cmd_add_channel(message):
    ...
    if ch in db["dest_channels"]:
        ...
        return
    if len(db["dest_channels"]) >= 10:
        bot.reply_to(message, "❌ সর্বোচ্চ ১০টা চ্যানেল।")
        return
...
def cmd_add_slot(message):
    ...
    for s in db["schedules"]:
        if s["time"] == ts:
            bot.reply_to(message, f"⚠️ <code>{ts}</code> এই সময়ে আগেই স্লট আছে।")
            return
...
def cmd_del_slot(message):
    parts = message.text.strip().split()
    if len(parts) < 2:
        bot.reply_to(message, "❌ ফরম্যাট: <code>/del_slot 1</code> (নম্বর দিয়ে)")
        return
    try:
        idx = int(parts[1]) - 1
    except:
...
def cmd_schedules(message):
    ...
    sch = db["schedules"]
    if not sch:
        bot.reply_to(message, "📭 কোনো শিডিউল নেই। /add_slot 07:30 4 দিয়ে যোগ করো।")
        return
...
def cmd_add_button(message):
    db = get_db()
    if len(db["buttons"]) >= 3:
        bot.reply_to(message, "❌ সর্বোচ্চ ৩টা বাটন। /del_button দিয়ে আগে মুছো।")
        return
...
def cmd_del_button(message):
    ...
    save_db(db)
    bot.reply_to(message, f"✅ <b>{removed['text']}</b> বাটন মুছে ফেলা হয়েছে।")
...
def cmd_buttons(message):
    ...
    btns = db["buttons"]
    if not btns:
        bot.reply_to(message, "📭 কোনো বাটন নেই। /add_button দিয়ে যোগ করো।")
        return
...
def cmd_queue(message):
    ...
    bot.reply_to(
        message,
        f"📦 <b>কিউ স্ট্যাটাস</b>\n\n"
        f"মোট: <b>{len(q)}</b> টি মিডিয়া\n"
        f"প্রতিদিন: <b>{per}</b> টি পোস্ট\n"
        f"আনুমানিক: <b>{len(q) // per if per else '∞'}</b> দিন",
        reply_markup=kb
    )
...
def do_post(count=1, notify=True):
    ...
    for _ in range(to_send):
        ...
        for ch in chs:
            try:
                ...
                log.error(f"Post error to {ch}: {e}")
                ok = False
                failed += 1
        if ok:
            sent += 1
    ...
    if notify and sent > 0:
        bot.send_message(
            ADMIN_ID,
            f"✅ <b>{sent}</b> টি পোস্ট হয়েছে!\n"
            f"📦 কিউতে বাকি: <b>{len(db['queue'])}</b> টি"
        )
        if len(db["queue"]) < 5:
            bot.send_message(
                ADMIN_ID,
                f"⚠️ <b>কিউ প্রায় খালি!</b> মাত্র <b>{len(db['queue'])}</b> টি বাকি।"
            )
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# MEDIA HANDLER (video/photo/doc/audio/animation)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def handle_media(message):
    ...
    db["queue"].append({
        "file_id": fid,
        "type":    ftype,
        "ts":      bdt_now().strftime("%Y-%m-%d %H:%M")
    })
    save_db(db)

    bot.reply_to(
        message,
        f"✅ <b>{ftype.capitalize()}</b> কিউতে যোগ হয়েছে!\n"
        f"📦 কিউতে মোট: <b>{len(db['queue'])}</b> টি"
    )
